...
@testset "Send" begin
  ...
  try
    ...
    let  # send with inline image
        message = html"""<h1>An important link to look at!</h1>
          <p>
          Here's an <a href="https://github.com/aviks/SMTPClient.jl">important link</a>
          </p>
          <p>
          And don't forget to check out the <strong>julia</strong> logo.
          </p>
          <p>
          <img src="cid:julia_logo_color.svg" alt="Julia Logo"/>
          </p>
        """
        ...
        body = get_body([addr], addr, subject, mime_message, attachments = attachments, multipart_subtype=SMTPClient.RELATED)

        send(server, [addr], addr, body)

        test_content(logfile) do s
          m = match(r"Content-Type:\s*multipart\/related;\s*boundary=\"(.+)\"[\r\n]+", s)
          @test m !== nothing
          boundary = m.captures[1]
          @test occursin("To: $addr", s)
          @test occursin("Subject: $subject", s)
          ...
          @test occursin("<strong>julia</strong>",s)
          @test occursin("<img src=\"cid:julia_logo_color.svg\" alt=\"Julia Logo\"/>",s)
          @test occursin("</body>", s)
          @test occursin("</html>", s)
          splt = split(s)
          ind = findall(v -> occursin("--$boundary", v), splt)
